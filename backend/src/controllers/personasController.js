...
const postPersonas = async (req, res) => {
    ...
    try {
        ...
        const respuestaBanco = await centralBank.post('/persons', {
            nombre,
            apellido,
            dni
        })
        const cbu = respuestaBanco.data.cbu
        console.log('CBU obtenido: ', cbu);

        await client.query('BEGIN');

        const respuestaPersona = await client.query(`
            INSERT INTO personas (nombre, apellido, dni, direccion, email, telefono, fecha_nac)
